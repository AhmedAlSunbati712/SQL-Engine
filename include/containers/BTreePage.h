...
class BInternalPage : public BTreePage {
    public:
        ...
        bool insert_separator_at(std::size_t idx, const Key &key, std::uint32_t right_child_page_num);
        bool remove_separator_at(std::size_t idx);
        bool set_separator_key_at(std::size_t idx, const Key &key);
        ...
        static void fill_initial_layout(char *out);

    protected:
        void decode() override;
        void flush() override;
        void parse_internal_cell(const char *in, Key *key, std::uint32_t *right_child_page_num);
...
class BLeafPage : public BTreePage {
    public:
        ...
        bool insert_at(std::size_t idx, const Key &key, const Value &value);
    ...
    protected:
        void decode() override;
        void flush() override;
        void parse_leaf_cell(const char *in, Key *key, Value *value);
